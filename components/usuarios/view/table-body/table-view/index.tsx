...
export default function TableView() {
  const { data, getSelectAllState, toggleAllSelect, isItemSelected, toggleSelectItem } = useGlobal()
            ...
            {data.items.map((item: any, index: any) => (
              <tr key={item.id} className={index % 2 === 0 ? "row-even" : "row-odd"}>
                ...
                {columns.map((column: any, Index: any) => {
                  ...
                  // Renderizar badge para 'role'
                  if (column.column === "role" && isValidRole(value)) {
                    return (
                      <td key={Index}>
                  ...
                  return <td key={Index}>{column.column === "acciones" ? <ActionRow item={item} /> : value || ""}</td>
                })}
              </tr>
            ))}
          </tbody>
