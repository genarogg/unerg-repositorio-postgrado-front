...
/* ===== TIPOS BÁSICOS ===== */
type UserRole = "SUPER" | "EDITOR" | "Todos"
type TrabajoStatus = "PENDIENTE" | "VALIDADO" | "RECHAZADO" | "Todos"
...
interface StatusBadges {
  PENDIENTE: StatusBadge
  VALIDADO: StatusBadge
  RECHAZADO: StatusBadge
}
...
interface GlobalFilterProps {
  ...
  setPeriodoAcademico: (periodo: string) => void
  setLineaDeInvestigacion: (linea: string) => void
  clearFilters: () => void
}
  ...
  devtools(
    (set) => ({
      ...
      badges: {
        ...
        estados: {
          PENDIENTE: { name: "PENDIENTE", color: "#f59e0b" },
          VALIDADO: { name: "VALIDADO", color: "#22c55e" },
          RECHAZADO: { name: "RECHAZADO", color: "#ef4444" },
        },
      } as BadgeConfig,
...
const useGlobalFilter = create<GlobalFilterProps>()(
  devtools(
    immer((set, get) => ({
      ...
      setPeriodoAcademico: (periodo: string) =>
        set((state) => {
          state.filterValue.periodoAcademico = periodo
        }),

      setLineaDeInvestigacion: (linea: string) =>
        set((state) => {
          state.filterValue.lineaDeInvestigacion = linea
        }),
